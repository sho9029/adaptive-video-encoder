...
NVIDIA GPU (NVENC) を活用した AV1 エンコードをデフォルトとし、VMAF または SSIM による自動画質保証機能を備えています。

## 特徴

*   **高圧縮・高画質**: デフォルトで AV1 (NVENC) コーデックを使用し、Visually Lossless（見た目の劣化なし）な設定でエンコードします。
*   **自動画質保証 (VMAF / SSIM)**: エンコード後に品質評価指標を計算し、基準値を下回った場合は自動的に画質設定を上げて再エンコードします（デフォルト設定: VMAF=93.0以上）。動画の解像度に合わせてVMAFモデル（通常用・4K用）を自動で切り替えます。
...
*   **FFmpeg**: NVENC 対応ビルド（パスが通っていること）
    ...
    *   **重要 (VMAFハードウェアアクセラレーション)**: `libvmaf_cuda` フィルタが有効化された**カスタムビルド版のFFmpeg**を使用している場合、ツールは自動的にそれを検知し、高速なCUDAによるVMAF計算を行います。標準的なFFmpeg（`libvmaf`有効化済）を使う場合は、自動的にCPUを利用した計算にフォールバックします。
...
*   `--metric`: 画質評価指標の指定 (デフォルト: `vmaf`)
    *   `vmaf`, `ssim`
*   `--min-score`: 自動再エンコードの基準となる品質閾値（下限）。デフォルトは指標ごと（VMAF: `93.0`, SSIM: `0.985`）
*   `--max-score`: 品質の上限閾値。これを超えると圧縮率を上げます。デフォルトは指標ごと（VMAF: `100.0`, SSIM: `1.0`）
*   `--max-retries`: 再エンコードの最大試行回数 (デフォルト: `5`)
...
**SSIMを指標として使用する（旧挙動）**
```powershell
python video_encoder.py "C:\Videos\Source" "C:\Videos\Encoded" --metric ssim
```

**VMAF基準値を厳しくする**
```powershell
python video_encoder.py "C:\Videos\Source" "C:\Videos\Encoded" --min-score 95.0
```
...
### 2. フィードバック制御による品質保証
エンコード完了後にソースファイルと生成ファイルを比較し、VMAF または SSIM を用いて品質を計測します。
